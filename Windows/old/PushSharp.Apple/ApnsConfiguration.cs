...
namespace PushSharp.Apple
{
    public class ApnsConfiguration
    {
        ...
        public ApnsConfiguration (ApnsServerEnvironment serverEnvironment, string certificateFile, string certificateFilePassword)
            : this (serverEnvironment, System.IO.File.ReadAllBytes (certificateFile), certificateFilePassword)
        {
        }

        public ApnsConfiguration (ApnsServerEnvironment serverEnvironment, byte[] certificateData, string certificateFilePassword)
            : this (serverEnvironment, new X509Certificate2 (certificateData, certificateFilePassword,
                X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable))
        {
        }

        public ApnsConfiguration(string overrideHost, int overridePort) : this(overrideHost, overridePort, true)
        {
        }

        public ApnsConfiguration (string overrideHost, int overridePort, bool skipSsl)
        {
            SkipSsl = skipSsl;
        ...
        void Initialize (ApnsServerEnvironment serverEnvironment, X509Certificate2 certificate)
        {
            ...
            AdditionalCertificates = new Collection<X509Certificate2> ();
        ...
        public Collection<X509Certificate2> AdditionalCertificates { get; private set; }
