...
;
        ...
        public void Check ()
        {
            ...
            try
            {
                client = new TcpClient(Configuration.FeedbackHost, Configuration.FeedbackPort);
                ...
                while (recd > 0)
                {
                    ...
                    while (data.Count >= (4 + 2 + 32)) // Minimum size for a valid packet
                    {
                        ...
                        if (data.Count >= 4 + 2 + tokenLength)
                        {
                            ...
                            var token = BitConverter.ToString(tokenBuffer).Replace("-", "").ToUpperInvariant().Trim();
                            ...
                            if (evt != null)
                                //evt (token, timestamp);                        
                                evt(this, new FeedbackReceivedEventArgs() { DeviceToken = token, Timestamp = timestamp });
                        }
                        else {
                            continue;
                        }
