...
namespace PushSharp.Apple
{
    public class ApnsNotification : INotification
    {
        ...
        internal const int DEVICE_TOKEN_BINARY_MIN_SIZE = 32;
        internal const int DEVICE_TOKEN_STRING_MIN_SIZE = 64;
        internal const int MAX_PAYLOAD_SIZE = 2048; //will be 4096 soon
        ...
        public ApnsNotification (string deviceToken, JObject payload)
        {
            if (!string.IsNullOrEmpty (deviceToken) && deviceToken.Length < DEVICE_TOKEN_STRING_MIN_SIZE)
                throw new NotificationException ("Invalid Device Token Length", this);
        ...
        public byte[] ToBytes ()
        {
            ...
            if (string.IsNullOrEmpty (this.DeviceToken))
                throw new NotificationException ("Missing Device Token", this);

            if (!IsDeviceRegistrationIdValid ())
                throw new NotificationException ("Invalid Device Token", this);

            // Turn the device token into bytes
            byte[] deviceToken = new byte[DeviceToken.Length / 2];
            for (int i = 0; i < deviceToken.Length; i++) {
                try {
                    deviceToken [i] = byte.Parse (DeviceToken.Substring (i * 2, 2), System.Globalization.NumberStyles.HexNumber, CultureInfo.InvariantCulture);
                } catch (Exception) {
                    throw new NotificationException ("Invalid Device Token", this);
                }
            }
