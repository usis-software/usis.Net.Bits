...
namespace usis.Framework
{
	...
	public class SnapInHost
	{
		public static void PresetSnapIn(Type type)
		{
			lock (presetSnapIns)
			{
				presetSnapIns.Add(type);
			}
		}

		#region fields

		private static Collection<Type> presetSnapIns = new Collection<Type>();
		...
		{
			// register pre-set snap-ins
			
			lock (presetSnapIns)
			{
				foreach (var item in presetSnapIns)
				{
					this.RegisterSnapIn(item);
				}
			}
